...
const IndividualFeedbackPage = async ({ params }: RouteParams) => {
  const { id: interviewId, feedbackId } = await params;
  const currentUser = await getCurrentUser();

  // Get the specific feedback record
  const feedback = await getFeedbackById(feedbackId);
  if (!feedback) {
    redirect(`/interview/${interviewId}/all-feedback`);
  }

  // Verify feedback belongs to this interview
  ...
  // Get the user who took the interview
  const interviewUser = await getUserById(feedback.userId);
  const userName = interviewUser?.name || feedback.userId;

  return (
    <section className="section-feedback">
      <div className="flex flex-row justify-center">
        <h1 className="text-4xl font-semibold">
          Feedback Report - {userName} -{" "}
      ...
      <p className="w-full text-center">
        Questions Attempted:
        <span className="font-bold">{feedback.attemptedCount || 0}</span> / {interview.questions?.length || 0}
      </p>
      ...
      <div className="buttons">
        <Button className="btn-secondary flex-1">
          <Link href={`/interview/${interviewId}/all-feedback`} className="flex w-full justify-center">
