...
export default function FeedbackReport({ feedback, interview, username }: any) {
  return (
    <section className="section-feedback">
      <div className="flex flex-row justify-center">
        <h1 className="text-4xl font-semibold">
          Feedback for {username} - {interview.role} Interview
        </h1>
      </div>
      ...
      <div className="mt-6 flex flex-col gap-3">
        ...
        <div className="transcript-container max-h-80 overflow-y-auto p-4 rounded-lg border border-gray-800 bg-black shadow-sm space-y-3">
          {feedback?.transcript?.map((entry: any, index: number) => (
            <div
              key={index}
